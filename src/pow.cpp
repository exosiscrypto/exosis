...
unsigned int GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock, const Consensus::Params& params)
{
    ...
    if (params.fPowNoRetargeting || params.fPowAllowMinDifficultyBlocks)
    {
        ...
        int blockstogoback2 = params.DifficultyAdjustmentInterval()-1;
        if ((pindexLast->nHeight+1) != params.DifficultyAdjustmentInterval())
            blockstogoback2 = params.DifficultyAdjustmentInterval();
...
unsigned int CalculateNextWorkRequired(const CBlockIndex* pindexLast, int64_t nFirstBlockTime, const Consensus::Params& params)
{
    ...
    if (nActualTimespan < params.nPowTargetTimespan/1.15)
        nActualTimespan = params.nPowTargetTimespan/1.15;
    if (nActualTimespan > params.nPowTargetTimespan*1.15)
        nActualTimespan = params.nPowTargetTimespan*1.15;
