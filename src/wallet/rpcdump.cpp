...
UniValue importaddress(const JSONRPCRequest& request)
{
...

    {
        ...
        if (IsValidDestination(dest)) {
            ...
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid Exosis address or script");
        }
    }
    if (fRescan)
    {
        ...
        while (file.good()) {
            ...
            if (vchSecret.SetString(vstr[0])) {
                ...
                if (pwallet->HaveKey(keyid)) {
                    LogPrint(BCLog::RPC, "Skipping import of %s (key already present)\n", EncodeDestination(keyid));
                    continue;
                }
                ...
                LogPrint(BCLog::RPC, "Importing %s...\n", EncodeDestination(keyid));
                if (!pwallet->AddKeyPubKey(key, pubkey)) {
                    fGood = false;
                    continue;
                }
               ...
               if (pwallet->HaveCScript(script)) {
                   LogPrint(BCLog::RPC, "Skipping import of %s (script already present)\n", vstr[0]);
                   continue;
               }
               if(!pwallet->AddCScript(script)) {
                   LogPrint(BCLog::RPC, "Error importing script %s\n", vstr[0]);
                   fGood = false;
                   continue;
               }
...
UniValue dumpprivkey(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"address\"   (string, required) The Exosis address for the private key\n"
            "\nResult:\n"
...
UniValue dumpwallet(const JSONRPCRequest& request)
{
    ...
    if (request.fHelp || request.params.size() != 1)
        throw std::runtime_error(
            ...
            "\nArguments:\n"
            "1. \"filename\"    (string, required) The filename with path (either absolute or relative to exosisd)\n"
            "\nResult:\n"
            "{                           (json object)\n"
    ...
    // produce output
    file << strprintf("# Wallet dump created by Exosis %s\n", CLIENT_BUILD);
                    ...
                    result.pushKV(
                        ...
                        JSONRPCError(
                            ...
                            strprintf("Rescan failed for key with creation timestamp %d. There was an error reading a "
                                      ...
                                      "caused by pruning or data corruption (see exosisd log for details) and could "
