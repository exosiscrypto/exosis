...
void CMasternodeSync::ProcessTick(CConnman& connman)
{
    ...
    if(GetTime() - nTimeLastProcess > 60*60) {
        LogPrint(BCLog::MNSYNC, "CMasternodeSync::HasSyncFailures -- WARNING: no actions for too long, restarting sync...\n");
        Reset();
    ...
    if(IsFailed()) {
        if(nTimeLastFailure + (1*60) < GetTime()) { // 1 minute cooldown after failed sync
            LogPrint(BCLog::MNSYNC, "CMasternodeSync::HasSyncFailures -- WARNING: failed to sync, trying again...\n");
            Reset();
            SwitchToNextAsset(connman);
        }
        return;
