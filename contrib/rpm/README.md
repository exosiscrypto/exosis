...
The RPM spec file provided here is for exosis-Core 0.16 and builds on CentOS
...
reference the release tarball available from https://exosis.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[exosis git repository](https://github.com/https://github.com/exosiscrypto/exosis) but are not part of
...
Currently only `Source10` is used, for the example `exosis.conf` file.
...
in the exosis git repository. At present only an SVG version of the exosis
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the exosis
developers so that an official fix to the issue can make it into the next
release.

### Patch0 exosis-0.16-libressl.patch
...
needed when 0.12.1 is released, a proper fix is already in the exosis git
master branch.
...
idea to build exosis Core against a static version of that library compiled
...
The `exosis.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `exosis-server` package. At this point, most RPM based
...
The `exosis-server` package should have SELinux support. How to properly do
...
exosis-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/exosis/). He has been
packaging exosis for Fedora at least since 2012.
...
4. I renamed the `exosis` package that contains the Qt GUI to `exosis-core` as
that appears to be how the general population refers to it, in contrast to
`exosis-xt` or `exosis-classic`. I wanted to make sure the general population
