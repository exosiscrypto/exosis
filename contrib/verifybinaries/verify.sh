...
###   exosiscore.org and exosis.org and compares them.
...
WORKINGDIR="/tmp/exosis_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://exosiscore.org"
HOST2="https://exosis.org"
BASEDIR="/bin/"
VERSIONPREFIX="exosis-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'exosis-',
...
if [ $? -ne 0 ]; then
   echo "exosis.org failed to provide signature file, but exosiscore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "exosis.org and exosiscore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Exosis Core binary release signing key installed?"
   fi
